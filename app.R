  ...
  shiny::shinyApp(
    ...
    server = function(input, output,session){
      ...
      observe({
        if(input$one == TRUE){  shell.exec("C:\\Users\\SeaportAI\\Desktop\\Pivo R database\\app.bat") }
        else if(input$two == TRUE){  shell.exec("C:\\Users\\SeaportAI\\Desktop\\outlier - with database\\app.bat")  }
        else if(input$three == TRUE){    shell.exec("C:\\Users\\SeaportAI\\Desktop\\anoboxcontrol level2 andpast - database\\app.bat")   }
        else{}
      })

      output$frame <- renderUI({
        address <- "https://seaportaiapps.shinyapps.io/ssss/"
          tags$iframe(src = address, height="1000",  width = "100%")
...
#       observe({
#         if(input$one == TRUE){
#           shell.exec("C:\\Users\\SeaportAI\\Desktop\\Pivo R database\\app.bat")
#                   }
#         else if(input$two == TRUE){
#           shell.exec("C:\\Users\\SeaportAI\\Desktop\\outlier - with database\\app.bat")
#                   }
#         else if(input$three == TRUE){
#           shell.exec("C:\\Users\\SeaportAI\\Desktop\\anoboxcontrol level2 andpast - database\\app.bat")
#               }
#         else {
#             }
#       })
...
# 
#         if(input$one == TRUE){
#           address <-"http://127.0.0.2:7099"
#           Sys.sleep(12)
#           tags$iframe(src=address, height=1000, width="100%")
#         }
#         else if(input$two == TRUE){
#           address <- "http://127.0.0.2:7100"
#           Sys.sleep(12)
#           tags$iframe(src=address, height=1000, width="100%")
#         }
#         else if(input$three == TRUE){
#           address <- "http://127.0.0.2:7101"
#           Sys.sleep(12)
#           tags$iframe(src=address, height=1000, width="100%")
# 
#         }
